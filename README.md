...
- `/usr/local/etc/xray/config.json`, the generated server config (mode 0600, since it contains the
  REALITY private key).
...
(Android Keystore-backed). Nothing is sent anywhere else, and there is no account or server-side
state to clean up.

## Building it

```sh
git clone --recurse-submodules github.com/amanxgawa/Nukemichi.git
```

`hev-socks5-tunnel` is a git submodule (native tun2socks), so the `--recurse-submodules` flag matters,
